...
export class EovService {
  ...
  async getDashboard(memberId: string) {
    try {
      if (!memberId) throw new BadRequestException('Missing authenticated user id');

      // 1) Load member and check if it's actually a member
      const member = await this.fetchUser(memberId);
      if ((member.roles ?? 'member') !== 'member') {
        throw new BadRequestException('EOV dashboard is only accessible to members, not sub-members.');
      }
      ...
      const totalSpending = memberTransactions.reduce((sum, tx) => 
      ...
      const subMemberFinances = await Promise.all(
        subMembers.map(sm => sm.financeId ? this.jsonServerService.getFinance(sm.financeId) : null)
      );
      ...
      const flaggedTransactions = memberTransactions.filter(tx => 
  ...
  async getSummaryReport(memberId: string, period?: string) {
    try {
      ...
      const member = await this.fetchUser(memberId);
      if ((member.roles ?? 'member') !== 'member') {
        throw new BadRequestException('EOV summary is only accessible to members, not sub-members.');
      }
      ...
      const totalSpending = filteredTransactions.reduce((sum, tx) => 
        sum + (Number(tx.bill) || 0), 0
      );

      const totalAllowance = await this.calculateTotalAllowance(member, subMembers);

      const flaggedTransactions = filteredTransactions.filter(tx => 
        tx.flagChargeId !== null && tx.flagChargeId !== undefined
      );
      ...
      const verifiedSpends = filteredTransactions.filter(tx => 
        tx.status === 'approved' && tx.verifyCharge === true
      );

      const flaggedCharges = filteredTransactions.filter(tx => 
        tx.flagChargeId !== null && tx.flagChargeId !== undefined
      );
  ...
  async sendEmailReport(memberId: string, period?: string, autoSend?: boolean) {
    try {
      ...
      const member = await this.fetchUser(memberId);
      if ((member.roles ?? 'member') !== 'member') {
        throw new BadRequestException('Email reports are only accessible to members, not sub-members.');
      }

      // 2) Check if member has email on file
      if (!member.email) {
        throw new BadRequestException('Member has no email address on file. Please update your profile with an email address.');
      }
   ...
   */
  async fetchAndGenerateReport(memberId: string, period?: string): Promise<StreamableFile> {
    if (!memberId) throw new BadRequestException('Missing authenticated user id');
    ...
    if ((member.roles ?? 'member') !== 'member') {
      ...
      if (!parentId) throw new BadRequestException('Sub-member has no parent member (parentId).');
    ...
    if (clubId === null || clubId === undefined || clubId === '') {
      throw new BadRequestException('User has no active club (currently_at is empty).');
    }
   ...
   */
  private filterTransactionsByPeriod(transactions: Transaction[], period?: string) {
    const now = new Date();
    let startDate: Date;
   ...
   */
  private async calculateTotalAllowance(member: User, subMembers: User[]): Promise<number> {
    try {
      const memberFinance = member.financeId ? await this.jsonServerService.getFinance(member.financeId) : null;
      const subMemberFinances = await Promise.all(
        subMembers.map(sm => sm.financeId ? this.jsonServerService.getFinance(sm.financeId) : null)
      ...
      const subMemberAllowance = subMemberFinances.reduce((sum, finance) => 
        sum + (finance?.totalAllowance || 0), 0
      );
   ...
   */
  private async getClubAndMemberBreakdown(member: User, subMembers: User[], transactions: Transaction[]) {
    try {
  ...
  private generateEmailContent(reportData: any, member: User): string {
    ...
    const timeBreakdownSection = breakdown.timeBased && breakdown.timeBased.length > 0 ? `
      ...
      <table border="1" style="border-collapse: collapse; width: 100%;">
        ...
        ${breakdown.timeBased.map((entry: any) => `
          <tr>
            <td>${entry.date}</td>
    ...
    return `
      ...
      <ul>
        ${Object.entries(breakdown.status).map(([status, count]) => 
          `<li>${status}: ${count} transactions</li>`
        ).join('')}
      </ul>
   ...
   */
  private async sendEmail(recipient: string, content: string): Promise<{ id: string }> {
  ...
  private async generatePDFReport(
    ...
    const safeMoveDown = (n = 0.5) => {
      try { doc.moveDown(n); } catch { /* ignore */ }
    };
    ...
    // Section helper (bold heading, then normal)
    ...
    if (!transactions.length) {
      ...
      doc.fontSize(10);
      const pageBottom = () => (Number((doc as any).page?.height) || 842) - 80;
    ...
    try {
      ...
      try {
        ...
        return allUsers.filter(
          (u) => String(u.parentId) === String(memberId) && (u.roles as string) === 'submember',
        );
      } catch {
  ...
  private async fetchTransactionsForClub(clubId: string): Promise<Transaction[]> {
    try {
  ...
  private async fetchFlagChargesForMember(memberId: string): Promise<FlagCharge[]> {
    try {
